...
Background (PowerShell, survives shell close):
    Start-Process wsl -ArgumentList `
      "-u owner bash -c 'cd /mnt/c/Personal/tickdock && python3 run_campaign.py'" `
      -WindowStyle Hidden
"""
...
from config import (
    ...
    VINA, KNOWN_PROMISCUOUS, BLACKLISTED_TARGETS, PRIMARY_SPECIES,
)
...
def load_pruned_cache(skip_near_misses: bool = False) -> None:
    ...
    Args:
        skip_near_misses: If True, promote all near-miss entries to exh=9999 so
            they are treated as permanent skips regardless of current exhaustiveness.
            Use when top-hit count is already high and near-miss re-docking would
            waste compute (--no-redock-near-misses flag).
    ...
    def _absorb(target: str, ligand: str, exh: int):
        if skip_near_misses and exh < 9999:
            exh = 9999   # treat near-miss as permanent skip
        key = (target, ligand)
    ...
    if n_total:
        ...
        near_str = (f"{n_near:,} near-misses [SKIPPED — --no-redock-near-misses]"
                    if skip_near_misses else
                    f"{n_near:,} near-misses [re-dockable at higher exh]")
        log(f"Pruned cache loaded: {n_total:,} entries ({n_perm:,} clear-fails [permanent], {near_str})")
...
_keepawake_ps_proc = None   # persistent PowerShell process holding ES_SYSTEM_REQUIRED
_keepawake_stop    = threading.Event()  # kept for stop_keepawake compat
...
# Strategy: powercfg /change standby-timeout-ac 0  — OS-level sleep disable.
# Previous approaches (SendKeys, SetThreadExecutionState) failed because
# Windows Modern Standby (S0) can override per-process execution state flags
# and ignores synthesized input.  powercfg changes the actual power plan
# timeout, which is the only reliable method.
# Original AC timeout is read and restored on stop.

_original_ac_timeout: int | None = None   # seconds; None = not yet saved


def _get_ac_sleep_timeout() -> int | None:
    """Read current AC standby timeout in seconds via powercfg."""
    try:
...
def start_keepawake():
    ...
    try:
        ...
        orig_str = f"{_original_ac_timeout}s" if _original_ac_timeout else "unknown"
        log(f"Keep-awake: AC sleep disabled via powercfg (was {orig_str})")
    except Exception as e:
        log(f"Keep-awake: powercfg failed ({e}) — system may sleep during run", "WARN")
...
def stop_keepawake():
    ...
    try:
        ...
        log(f"Keep-awake: AC sleep restored to {restore_min}min via powercfg")
    except Exception as e:
        log(f"Keep-awake: restore failed ({e}) — manually run: powercfg /change standby-timeout-ac 15", "WARN")
    _keepawake_stop.set()
    if _keepawake_ps_proc and _keepawake_ps_proc.poll() is None:
        _keepawake_ps_proc.terminate()
...
def prep_receptor(target: str) -> str | None:
    ...
    # Primary: -xr + pH + gasteiger. Gasteiger fails to kekulize some experimental
    # RCSB structures (e.g. B7QK46) → obabel exits 0 but writes a 0-byte file.
    # Detect (size>100 guard) and fall back to bare -xr, which succeeds.
    attempts = [
        ("gasteiger", ["obabel", pdb_path, "-O", out_path,
                       "-xr", "-p", str(VINA["ph"]),
                       "--partialcharge", "gasteiger", "--quiet"]),
        ("bare -xr",  ["obabel", pdb_path, "-O", out_path, "-xr"]),
    ]
    last_reason = "unknown"
    for label, cmd in attempts:
        try:
            ...
            last_reason = f"receptor prep ({label}) error: {e}"
            log(f"{target}: {last_reason}", "WARN")
            continue
        sz = os.path.getsize(out_path) if os.path.exists(out_path) else 0
        if result.returncode == 0 and sz > 100:
            if label != "gasteiger":
                log(f"{target}: gasteiger failed; receptor via {label} ({sz}B)", "WARN")
            return out_path
        last_reason = (f"obabel {label} gave {sz}B "
                       f"(exit {result.returncode}): {result.stderr.decode()[:100]}")
        log(f"{target}: {last_reason}", "WARN")
    _record_receptor_failure(target, last_reason)
    return None
...
def run_post_campaign(top_targets: int = 25, skip_orthologs: bool = False,
    ...
    steps = [
        ("Promiscuous filter",
         ...
         600),   # full 139-target × 100k+ hit scan needs > 120s at full library size
    ...
    steps += [
        ...
        ("Generate figures",
         ...
         900),   # load_docking_data scans result PDBQTs on /mnt/c — slow at full scale
    ...
    for name, cmd, timeout in steps:
        ...
        # After promiscuous filter patches config.py, immediately rebuild top_hits.json
        # with reloaded KNOWN_PROMISCUOUS so downstream steps see the clean file.
        if "promiscuous" in name.lower():
            log("  Rebuilding top_hits.json with updated promiscuous filter...")
            try:
                n = rebuild_top_hits()
                log(f"  Rebuild top hits: OK ({n:,} clean hits)")
            except Exception as e:
                log(f"  Rebuild top hits: {e}", "WARN")
...
def rebuild_top_hits() -> int:
    ...
    # Reload KNOWN_PROMISCUOUS and BLACKLISTED_TARGETS fresh from config so that
    # check_promiscuous.py --update-config patches take effect without restarting.
    try:
        import importlib, config as _cfg
        importlib.reload(_cfg)
        _known_promiscuous   = _cfg.KNOWN_PROMISCUOUS
        _blacklisted_targets = _cfg.BLACKLISTED_TARGETS
    except Exception:
        _known_promiscuous   = KNOWN_PROMISCUOUS
        _blacklisted_targets = BLACKLISTED_TARGETS

    seen: dict[tuple, float] = {}  # (target, ligand) -> best score
    n_blacklisted_skipped = 0
    for pat in ("batch_*_compressed.json", "batch_R*_B*_compressed.json"):
        for path in glob.glob(os.path.join(LOG_DIR, pat)):
            try:
                data = json.load(open(path))
                for h in data.get("kept", []):
                    if h["target"] in _blacklisted_targets:
                        n_blacklisted_skipped += 1
                        continue
                    if h["ligand"] in _known_promiscuous:
                        continue
                    key = (h["target"], h["ligand"])
                ...
                pass
    if n_blacklisted_skipped:
        log(f"rebuild_top_hits: skipped {n_blacklisted_skipped} hits from {len(_blacklisted_targets)} blacklisted target(s)")
    if not seen:
        return 0
    hits = sorted(
        [{"target": t,
...
def show_status():
    ...
    if all_tops:
        # Filter blacklisted targets from status display
        all_tops_clean = [h for h in all_tops if h.get("target") not in BLACKLISTED_TARGETS]
        n_blacklisted = len(all_tops) - len(all_tops_clean)
        bl_note = f" ({n_blacklisted} blacklisted target hits hidden)" if n_blacklisted else ""
        print(f"\n  Best hits so far ({len(all_tops_clean)} clean hits{bl_note}):")
        for i, h in enumerate(all_tops_clean[:5], 1):
...
def main():
    ...
    parser.add_argument("--no-redock-near-misses", action="store_true",
                        help="Treat near-misses as permanent skips (exh=9999); prevents "
                             "re-docking the -5.5 to -7.0 zone even at higher exhaustiveness. "
                             "Use when top-hit count is already high and near-miss compute "
                             "would be wasteful.")
    ...
    load_pruned_cache(skip_near_misses=args.no_redock_near_misses)
