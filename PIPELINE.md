...
## GPU Docking (Vina-GPU 2.1, AMD RDNA4) — primary engine

~5× faster than the 16-core CPU campaign, same AutoDock Vina scoring (validated ≡
CPU within ±0.5 kcal/mol). Runs **Windows-native** (WSL2 can't see the AMD GPU);
`gpu_screen.py` runs in WSL and launches the Windows exe via interop. Full build +
patch details: **[docs/gpu_docking.md](docs/gpu_docking.md)**, `tools/gpu_build/`.

```bash
# 1. Build/grow the ligand library to a GUARANTEED valid count (RDKit ETKDG + Meeko,
#    pH-7.4 protonated; ~0.05s/mol, loops until target reached)
python scripts/build_library.py --target 10000 --workers 16
python scripts/build_library.py --status          # current valid count

# 2. (optional) uniformly re-prep the whole library with one pipeline
python scripts/reprep_library.py --workers 16

# 3. Validate the GPU engine reproduces CPU before trusting a run
python scripts/validate_gpu.py --n 15             # redock top hits on GPU vs CPU scores

# 4. Screen on GPU (integrates into pruned_nonhits.jsonl + top_hits.json)
python scripts/gpu_screen.py --targets B7P5E9 B7PY20   # specific targets
python scripts/gpu_screen.py --all                     # all 138 (BLACKLISTED excluded)
python scripts/gpu_screen.py --targets B7P5E9 --dry-run

# Graceful pause (finishes current 512-chunk, saves, stops) + resume
python scripts/gpu_screen.py --stop                    # signal a running instance
python scripts/gpu_screen.py --all                     # re-run = resume (skips done)
```

Robustness: only valid (>500B) ligands; multi-model salts reduced to largest
fragment; metal/exotic-atom ligands skipped (they crash Vina-GPU); chunk(512) +
bisect isolates any remaining crash-inducing ligand; per-chunk checkpoint (a
pause/kill loses ≤1 chunk); keep-awake + staging-overlap (prefetch next target's
CPU prep while the GPU docks the current one). Configs use Windows (`C:/`) paths.

Launch persistent (survives shell close):
```powershell
Start-Process wsl -ArgumentList "-u owner bash -c 'cd /mnt/c/Personal/tickdock && python3 scripts/gpu_screen.py --all 2>&1 | tee logs/gpu_redock_full.log'" -WindowStyle Hidden
```

---

## Campaign Orchestrator (CPU — legacy / parallel)
...
## Co-folding Validation (Boltz-2 — NOT AlphaFold Server)

Independent protein–ligand pose (and affinity) prediction to cross-validate the
top Vina hits.

> ⚠️ **Do NOT use AlphaFold Server.** Its terms forbid using outputs "in any
> automated system that predicts the binding or interaction of the protein with
> ligands … such as Glide or AutoDock" and are **non-commercial only** — both
> hit this exact workflow. Use **Boltz-2 (MIT)** instead: no commercial / no
> automated-binding restriction, runs on free Colab GPU, and also predicts affinity.

```bash
# 1. Prep inputs (protein seq + ligand SMILES per top hit) — still useful: Boltz
#    reuses the same inputs. (The old "submit at alphafoldserver" note is retired.)
python scripts/prep_af3_jobs.py --auto-targets 3 --top 5
python scripts/prep_af3_jobs.py --incremental --dry-run

# 2. Run Boltz-2 on Modal cloud A100 (free-tier credits; folds all leads in one run):
#    modal run scripts/boltz_modal.py   (see docs/boltz_HOWTO.md)
#    Results auto-sorted -> data/docking/af3_compare/<TARGET>_<LIGAND>_cofold/
#    (Old free-Colab path is legacy — Colab T4 OOMs on the largest lead.)

# 3. Compare predicted pose vs the Vina pose (superpose protein Ca -> ligand RMSD)
python scripts/compare_cofold_vina.py --target B7P5E9 --ligand CHEMBL94259
```

Low ligand RMSD (<2 Å) + good Boltz iptm/ligand_iptm = two independent methods
agree on the binding pose → strong corroboration. Boltz-2's affinity estimate is
a bonus orthogonal check. `prep_af3_jobs.py` JSONs hold the reusable seq+SMILES;
`compare_cofold_vina.py` handles Boltz (or AF3) output interchangeably.
...
| GPU (AMD RX 9070 XT) | RDNA 4 (gfx1201) | Vina-GPU 2.1, Windows-native (not WSL2); now primary engine via `scripts/gpu_screen.py` |
