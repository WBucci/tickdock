...
**Draft manuscript — v1, 2026-07-23**
**Status:** internal draft. Author/affiliation TBD. All numbers trace to `docs/phase0_findings.md` and the `logs/*_calibration.json` files.
...
- **The PDB-coverage confound is RESOLVED — assert the anti-correlation as biology (control run 2026-07-23).** The full triage over 48,767 dockable tick proteins gives an 11-fold anti-correlation between pollinator-absence and structural-template availability (profile rate 0.98% vs 10.47% expected under independence). The concern was that "has a PDB template" encodes "is studied/conserved," making the effect a study-bias artifact. The control substitutes a **study-agnostic** structural measure — AlphaFold pLDDT, which is assigned to every protein regardless of research interest — for PDB presence. On a random 180-per-group sample of *I. scapularis*: bee-absent proteins have **mean pLDDT 62.0 vs 76.0** for bee-present (gap +14.0, permutation p < 0.0001), are **59.2% disordered vs 31.3%**, and are **well-folded (pLDDT≥70) only 37% of the time vs 72%**. AlphaFold-DB coverage barely differs (82% vs 91%), so it is not a database-presence artifact — the proteins are *in* the database; AlphaFold simply cannot fold them. The anti-correlation therefore reflects genuine biology: pollinator-absent (tick-specific) proteins are disproportionately intrinsically disordered secreted effectors that lack druggable pockets. PDB coverage bias may amplify the effect but is not its cause. Result in `logs/alphafold_coverage_control.json`.
- Decide the framing: pure methods paper (drop most of §5's tick specifics to a short vignette) vs methods + application. The methods result is the stronger and more general contribution; §5 is the memorable one. Current draft keeps both; a referee may want §5 trimmed.
- Get a second monomeric bee-sparing or arthropod-selective case if one exists, to shore up §3.1 / limitations.
- Target venue: a methods or cheminformatics journal (JCIM, J. Cheminformatics) or a preprint first. The reusable benchmark is the hook.
- Verify every PDB/ligand code one more time against RCSB at submission; they were verified 2026-07-22 but depositions change.

---

## Citation map — first pass (2026-07-23)

Every external claim requiring a reference, by location. `[n]` = reference number below. Claims backed only by this study's own data (the four calibration results, the triage, the pLDDT control) need no external citation and are not listed. **Status (2026-07-24): all references verified against NCBI E-utilities / CrossRef / RCSB and inserted inline; zero placeholders remain. Two caveats survive at the end of the reference list (the trimethoprim fold-number and PDB 2W3A) and must be honored at submission.**

| # | Claim | Location |
|---|---|---|
| 1 | Trimethoprim is ~10^4–10^5-fold selective for bacterial over human DHFR | Abstract; §2.2 |
| 2 | Methotrexate binds human and bacterial DHFR comparably (cross-reactive) | Abstract; §2.2 |
| 3 | Pyrimethamine selectivity for *Plasmodium* over human DHFR | §2.2 |
| 4 | Trimethoprim resistance via *S. aureus* DHFR F98Y (residue-level mechanism) | §2.2 |
| 5 | Amitraz mite-vs-bee selectivity arises from ~3 octopamine-receptor binding-site residues | §2.1; §4 |
| 6 | RDL / GABA-gated chloride channel is the fipronil target | §2.1 |
| 7 | Fipronil binds the M2 channel-pore region (inter-subunit) | §3.2 |
| 8 | GluCl / glutamate-gated chloride channel is the avermectin target | §2.1 |
| 9 | nAChR is the neonicotinoid target | §2.1 |
| 10 | AChE is the organophosphate/carbamate target | §2.1 |
| 11 | Binding/active sites are, on average, more conserved than whole-protein sequence | §4 |
| 12 | AlphaFold pLDDT is a validated proxy for intrinsic disorder | §4 (disorder control) |
| 13 | Tick salivary/secreted effectors (cystatins etc.) act by protein–protein interaction, lacking small-molecule pockets | §4; §5 |
| 14 | Docking scoring functions have ~2–3 kcal/mol error vs experiment / are not designed for cross-receptor comparison | §3.4; §4 |
| 15 | AI/physics docking frequently yields physically invalid or non-generalizing poses (supports the limitation framing) | §5 (optional) |
| 16 | FEP achieves ~1 kcal/mol accuracy | §7 |
| 17 | DHFR is an established antifolate drug target (decades of pharmacology) | §6 |
| S1 | AutoDock Vina | §2.3; §8 |
| S2 | fpocket | §2.3 |
| S3 | AlphaFold (model) + AlphaFold DB | §2.3; §4 |
| S4 | BLAST / BLASTP | §2.3 |
| S5 | Needleman–Wunsch global alignment | §2.3 |
| S6 | BLOSUM62 substitution matrix | §2.3 |
| S7 | Biopython (PairwiseAligner) | §2.3 |
| S8 | RDKit / Meeko (ligand prep) | §2.3 (M4) |
| P | PDB depositions: 7NAE, 2W3A, 2W9H, 2BL9, 4M6K, 1RG7, 1U72, 6P9Z | §2.2 |
...
### Two verification caveats (must survive to submission)

- **Ref 6 / the trimethoprim fold-selectivity number.** Burchall & Hitchings 1965 is the head-to-head bacterial-vs-human DHFR inhibitor comparison and is the correct primary citation. The specific "~30,000-fold" figure used in earlier drafts is a *later-review summarization* of that Ki data, **not** a verbatim value in the 1965 paper. The manuscript has been reconciled to state trimethoprim's selectivity as spanning the commonly-quoted 10³–10⁵-fold range and to frame the argument at the conservative end (10³-fold ≈ 4 kcal/mol via ΔΔG = RT·ln(ratio), RT ≈ 0.59 kcal/mol at 298 K), which still exceeds the observed ≤0.57 kcal/mol by ~7×. If a primary source for a precise fold-number is required, it must be found before that number is reinstated.
- **PDB 2W3A.** The structure is real (human DHFR·NADPH·trimethoprim, confirmed via RCSB Data API) but its RCSB "primary citation" field is a curation error pointing to an unrelated unpublished *M. avium* DHFR entry. **Cite 2W3A by PDB ID only; do not attach a journal reference to it.** The other seven PDB IDs have valid primary citations in RCSB.
- **F98Y note.** Ref 8 (Dale 1997) is cited for the *residue-level mechanism* of *S. aureus* trimethoprim resistance, not for a structure in the benchmark — the *S. aureus* DHFR structures used (2W9H, 6P9Z) are wild-type. No F98Y structure is docked or cited.
