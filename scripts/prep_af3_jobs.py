...
def make_af3_json(job_name: str, target_name: str, sequence: str,
    ...
    return {
        ...
        "_meta": {
            ...
            "note":             (
                "DO NOT submit to AlphaFold Server (terms forbid binding-prediction "
                "use with AutoDock + non-commercial only). Use Boltz-2 (MIT) instead: "
                "docs/boltz_cofold_colab.md reuses this 'sequence' + 'smiles'. "
                "The _meta block is local reference only."
            ),
        }
    }
...
def write_submission_guide(jobs: list[dict], out_dir: str):
    ...
    lines = [
        "Co-folding Submission Guide — USE BOLTZ-2, NOT AlphaFold Server",
        "=" * 60,
        f"Total jobs: {len(jobs)}",
        "",
        "*** AlphaFold Server is OFF-LIMITS for this pipeline ***",
        "  Its terms forbid using outputs in automated binding-prediction systems",
        "  'such as Glide or AutoDock', and are non-commercial only. Both apply here.",
        "",
        "Use Boltz-2 (MIT) — license-clean, free Colab GPU, also predicts affinity:",
        "  1. Open docs/boltz_cofold_colab.md (copy-paste Colab cells)",
        "  2. Per job: paste the sequence + SMILES below into Cell 2",
        "  3. Run (GPU runtime) — ~5-10 min/job",
        "  4. Download result -> data/docking/af3_compare/{job_name}_cofold/",
        "  5. python3 scripts/compare_cofold_vina.py --target T --ligand L",
        "",
