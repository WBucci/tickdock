...
from config import RESULTS_DIR, DOCKING_DIR, BASE_DIR, KNOWN_PROMISCUOUS, BLACKLISTED_TARGETS, VINA
...
def main():
    ...
    # Print top 10 (cross-species, blacklisted targets excluded)
    ranked = sorted(scores.items(), key=lambda x: x[1]["best_score"])
    blacklisted_found = [(a, e) for a, e in ranked if a in BLACKLISTED_TARGETS and e["best_score"] < 0]
    ranked_clean = [(a, e) for a, e in ranked if a not in BLACKLISTED_TARGETS]
    print(f"\nTop 10 targets by best score (promiscuous + blacklisted excluded):")
    print(f"  {'Target':<18} {'Best (kcal/mol)':>16}  {'Ligand':<20}  {'N hits':>6}")
    print(f"  {'-'*65}")
    for acc, entry in ranked_clean[:10]:
    ...
    if blacklisted_found:
        print(f"\n  [BLACKLISTED — excluded from ranking]:")
        for acc, entry in blacklisted_found:
            n = len([s for s in entry["scores"] if s <= VINA.get("good_score", -7.0)])
            reason = BLACKLISTED_TARGETS.get(acc, "")[:60]
            print(f"  {acc:<18} {entry['best_score']:>16.3f}  {entry['best_ligand']:<20}  {n:>6}  ({reason}...)")
