...
import sys, os, json, time, datetime, argparse, subprocess, threading
...
# ── ChEMBL offset high-water tracking ──────────────────────────────────────────
# Persist the deepest raw-pagination offset consumed so future forward downloads
# resume past it instead of re-fetching already-seen compounds. Avoids the
# manual log-scraping that the Round-5 download needed.
OFFSET_STATE_FILE = os.path.join(DOCKING_DIR, "chembl_offset_state.json")


def read_offset_state() -> int:
    """Return the max ChEMBL raw offset consumed so far (0 if no record)."""
    try:
        with open(OFFSET_STATE_FILE) as f:
            return int(json.load(f).get("max_offset_consumed", 0))
    except Exception:
        return 0


def write_offset_state(offset: int, mode: str = "druglike"):
    """Record max(existing, offset) as the new high-water mark."""
    prev = read_offset_state()
    try:
        with open(OFFSET_STATE_FILE, "w") as f:
            json.dump({"max_offset_consumed": max(prev, int(offset)),
                       "mode": mode,
...
def fetch_chembl_compounds(target_count: int, start_offset: int = 0,
    ...
    while len(compounds) < fetch_goal:
        ...
        if len(compounds) >= fetch_goal:
            break
    # Persist high-water offset so future forward runs resume past it.
    write_offset_state(offset, mode)
    return compounds


# obabel --gen3d conformer search timeout (seconds). Generous so a slow search
# under CPU contention degrades to "slower" instead of a false "failed". The
# Round-5 download lost ~99% of conversions to a 60s timeout while vina was
# saturating all cores — every kill was logged as an indistinct "failed".
CONVERT_TIMEOUT = 120
...
def _convert_worker(args: tuple) -> tuple[str, bool, str]:
    ...
    args: (smiles, zinc_id, out_path, ph[, timeout])
    Returns (zinc_id, success, reason). reason is "" on success, else one of:
      "timeout"  — gen3d exceeded the timeout (usually CPU starvation, retryable)
      "obabel:N" — obabel exited non-zero (N = returncode; parse/charge failure)
      "invalid"  — obabel exited 0 but produced an invalid/empty PDBQT
      "exc:..."  — unexpected exception
    """
    smiles, zinc_id, out_path, ph = args[0], args[1], args[2], args[3]
    timeout = args[4] if len(args) > 4 else CONVERT_TIMEOUT
    try:
        # nice -n 19: lowest scheduling priority. If a download overlaps a running
        # vina campaign (both CPU-bound, equal core count), this makes obabel yield
        # to vina — gen3d just runs slower instead of starving and timing out, and
        # the docking campaign is left undisturbed. No-op when nothing competes.
        cmd = [
            "nice", "-n", "19",
            "obabel", "-ismi", "-opdbqt",
            "-O", out_path,
        ...
        if result.returncode != 0:
            # obabel often creates a 0-byte -O file before failing — remove it so
            # empty stubs don't accumulate in the library.
            if os.path.exists(out_path):
                try: os.unlink(out_path)
                except OSError: pass
            return zinc_id, False, f"obabel:{result.returncode}"
        if not validate_pdbqt(out_path):
            return zinc_id, False, "invalid"
        return zinc_id, True, ""
    except subprocess.TimeoutExpired:
        if os.path.exists(out_path):
            try: os.unlink(out_path)
            except OSError: pass
        return zinc_id, False, "timeout"
    except Exception as e:
        return zinc_id, False, f"exc:{type(e).__name__}"
...
def download_and_prep(target_count: int, ligands_dir: str,
    ...
    ok_count   = 0
    fail_count = 0
    reasons    = {}          # reason → count, for a post-run breakdown
    timed_out  = []          # (smiles, zinc_id, out_path, ph) to retry serially
    _lock      = threading.Lock()

    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {pool.submit(_convert_worker, args): args for args in work}
        for future in as_completed(futures):
            zinc_id, ok, reason = future.result()
            with _lock:
                done_count += 1
                if ok:
                    ok_count   += 1
    ...
    # Failure breakdown — names *why* each conversion died instead of a blind
    # "failed". timeout = CPU starvation (retryable); obabel:N / invalid = the
    # compound itself (not worth retrying).
    if reasons:
        breakdown = ", ".join(f"{k}={v}" for k, v in sorted(reasons.items(),
                                                             key=lambda kv: -kv[1]))
        print(f"\n  Failure breakdown: {breakdown}")

    # ── Phase 2b: Serial retry of timeouts ───────────────────────────────────
    # Timeouts are almost always CPU contention, not bad chemistry. Retry them
    # one-at-a-time (no worker contention) with a doubled timeout — recovers the
    # bulk of a starved run without re-fetching anything.
    if timed_out:
        print(f"\n  Retrying {len(timed_out)} timed-out conversions serially "
              f"(timeout {CONVERT_TIMEOUT * 2}s, no contention)...")
        retry_ok = 0
        for i, args in enumerate(timed_out, 1):
            _, ok, reason = _convert_worker((args[0], args[1], args[2], args[3],
                                             CONVERT_TIMEOUT * 2))
            if ok:
                retry_ok += 1
                prepared += 1
                ...
                skipped  += 1
            if i % 25 == 0 or i == len(timed_out):
                print(f"    retry {i}/{len(timed_out)} — {retry_ok} recovered")
        print(f"  Serial retry recovered {retry_ok}/{len(timed_out)} "
              f"({len(timed_out) - retry_ok} still failed)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-offset", type=int, default=None, metavar="N",
                        help="Skip first N raw ChEMBL compounds; use to extend library "
                             "without re-fetching already-seen compounds. "
                             "DEFAULT (omitted): auto-resume from the recorded high-water "
                             "offset in data/docking/chembl_offset_state.json. Pass 0 to "
                             "force a re-walk from the start (e.g. for a widened-filter "
                             "backfill of already-traversed offsets).")
    parser.add_argument("--workers", type=int, default=0, metavar="N",
                        help="Parallel obabel workers for PDBQT conversion "
                             "(default: cpu_count). 16 workers = ~16x speedup over serial.")
    args = parser.parse_args()

    # Auto-resume offset when --start-offset omitted (None). Pass 0 explicitly to
    # force a re-walk from the start (widened-filter backfill of old offsets).
    if args.start_offset is None:
        args.start_offset = read_offset_state()
        if args.start_offset:
            print(f"Auto-resume: start-offset {args.start_offset} "
                  f"(from {OFFSET_STATE_FILE})")
