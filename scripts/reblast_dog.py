...
from config import (BLAST_HOSTS, RESULTS_DIR, LOG_DIR, REQUEST_DELAY, REQUEST_TIMEOUT,
                    MAX_HUMAN_HOMOLOGY, host_risk_label)
...
def main():
    ...
    for i, tgt in enumerate(targets, 1):
        ...
        if vals:
            ...
            # NOTE (fixed 2026-07-22): this previously wrote a BOOLEAN
            #   human_risk = max(vals) > MAX_HUMAN_HOMOLOGY
            # which was wrong twice over: `human_risk` is a STRING label
            # everywhere else (see _human_risk_label in 03_to_07), and the
            # comparison used MAX_HUMAN_HOMOLOGY (0.40) rather than the
            # HIGH tier (0.60). Because this script rewrites final_targets.json
            # in place, it clobbered the string labels on every target it
            # touched, leaving ~32 targets above the HIGH threshold carrying a
            # bare `True` or a stale "MEDIUM" and therefore never receiving the
            # -5 HIGH-homology score penalty.
            tgt["blast_result"]["human_risk"] = host_risk_label(max(vals))
