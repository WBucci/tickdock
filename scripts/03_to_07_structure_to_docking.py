...
def _human_risk_label(identity: float) -> str:
    """Thin wrapper — the canonical implementation now lives in config so that
    this module and reblast_dog.py cannot drift apart again (they did: one
    wrote this string label, the other a bare boolean against the wrong
    threshold, and reblast_dog rewrote final_targets.json in place)."""
    return host_risk_label(identity)
...
def compute_final_score(p: dict, blast: dict, rnai: dict) -> tuple[int, list]:
    ...
    if ident is not None:
        ...
        elif ident >= HIGH_HUMAN_HOMOLOGY: score -= 5; reasons.append(f"HIGH human homology {ident*100:.0f}% (-5)")
...
def run_step5(proteins: list[dict], skip_blast: bool,
    ...
    log.stat("high_human_homology_flagged", high_risk,
             f"Proteins with ≥{int(HIGH_HUMAN_HOMOLOGY*100)}% human sequence identity (HIGH risk, deprioritized)")
