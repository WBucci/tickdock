...
def load_top_hits(n: int = 30) -> list[dict]:
    ...
    """
    all_hits = []
    # GPU pipeline writes data/docking/top_hits.json (already promiscuous/blacklist
    # filtered by rebuild_top_hits) as a flat list of {target,ligand,score,round}.
    th_path = os.path.join(DOCKING_DIR, "top_hits.json")
    if os.path.exists(th_path):
        try:
            for rec in json.load(open(th_path)):
                if rec.get("ligand", "") in KNOWN_PROMISCUOUS:
                    continue
                if rec.get("score", 0) <= VINA["good_score"]:
                    all_hits.append(rec)
        except Exception as e:
            print(f"  [WARN] Cannot read {th_path}: {e}")
    else:
        # legacy CPU-era fallback
