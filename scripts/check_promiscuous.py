...
from config import DOCKING_DIR, KNOWN_PROMISCUOUS, PROMISCUOUS_THRESHOLD, BLACKLISTED_TARGETS
...
def main():
    ...
    # Save clean top_hits (top_hits.json filtered): drop this-run promiscuous,
    # already-known promiscuous, AND blacklisted targets (COX1 etc.). clean_hits.json
    # feeds generate_figures + cross_species_orthologs, so it must be fully clean.
    top_hits = load_top_hits(DOCKING_DIR)
    clean_hits = [h for h in top_hits
                  if h.get("ligand", "").replace("_out", "") not in flagged
                  and h.get("ligand", "").replace("_out", "") not in KNOWN_PROMISCUOUS
                  and h.get("target") not in BLACKLISTED_TARGETS]
