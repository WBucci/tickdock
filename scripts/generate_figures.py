...
def load_docking_data() -> dict:
    """
    Returns dict: target -> {scores: [float,...], best: float|None, n_hits: int}

    Reads top_hits.json (single fast file read) instead of scanning every result
    PDBQT across all *_results dirs. The old PDBQT scan walked hundreds of
    thousands of files on /mnt/c and repeatedly timed out the figures step.
    top_hits.json holds all qualifying hits — deduplicated best-per-(target,
    ligand) across rounds, with blacklisted targets and promiscuous ligands
    already removed — so it is both faster and MORE complete than the on-disk
    scan (which omits hits that were compressed off disk). Distribution is
    hit-only (≤ good_score) in both cases, so figures are unchanged.

    Falls back to the legacy PDBQT scan only if top_hits.json is missing.
    """
    top_path = os.path.join(DOCKING_DIR, "top_hits.json")
    if os.path.exists(top_path):
        ...
        try:
            ...
            for h in hits:
                ...
                s = h.get("score")
                lig = h.get("ligand", "")
                if t is None or not isinstance(s, (int, float)):
                    continue
                if t in BLACKLISTED_TARGETS or lig in KNOWN_PROMISCUOUS:
                    continue
                per_target.setdefault(t, []).append(s)
            return {t: {"scores": sorted(ss),
                        "best":   min(ss) if ss else None,
                        "n_hits": sum(1 for s in ss if s <= HIT_THRESH)}
                    for t, ss in per_target.items()}
        except Exception:
            pass  # fall through to legacy scan

    # Legacy fallback: scan result PDBQTs (slow; only if top_hits.json absent)
